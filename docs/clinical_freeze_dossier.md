...
| D027 | Gradient boosting por histogramas como comparador no lineal, inicialmente sobre las mismas cinco variables; imputación/indicadores dentro de fold y sin early stopping interno no agrupado. Congelar hiperparámetros o espacio de tuning antes del test. | Complejidad reducida; manejo nativo de missingness; omitir comparador si la información es insuficiente. | Estadística | pendiente de tamaño muestral y tuning preespecificado |
| D029 | Tras congelar el modelo, ajustar en validation una recalibración logística de intercepto y pendiente solo si existen al menos 100 pacientes con evento y 100 sin evento. El test solo evalúa coeficientes ya congelados. | Actualización solo de intercepto; no recalibrar si la calibración y la información son adecuadas; ampliar validation antes de usar estimaciones inestables. | Estadística | pendiente de información completa y firma |
