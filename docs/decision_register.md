...
| D020 | frozen | La ruta inicial para los CSV completos usa DuckDB fuera de memoria: selecciona la cohorte primero, aplica pushdown por estancia/itemid/tiempo y materializa Parquet con columnas mínimas, manifiesto, hash y reanudación verificable. | `src/mimic_sepsis/full_extract.py`; `scripts/extract_full_mimic.py` | La extracción se validó en el demo con límite de 1 GB; el dimensionado y rendimiento deben medirse en el servidor completo. |
